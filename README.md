...
```powershell
# 单策略
uv run qkquant backtest momentum --start 2023-01-01 --end 2025-12-31 --capital 30000

# 横向对比生产策略
uv run qkquant compare momentum ma_boll --start 2023-01-01 --end 2025-12-31
```
...
定时调度（Windows 任务计划程序，每个交易日 16:30 自动跑）：
...
| 策略 | 核心逻辑 | 近一年收益 | Sharpe | MDD | 定位 |
|---|---|---|---|---|---|
| `momentum` | 20日累计涨幅 > +3% & 距高点 < 10%（默认最多 5 只，单票 ≤22% 与一手取整对齐） | +7.26% | 0.35 | 16.74% | 观察/辅助信号 |
| `ma_boll` | 双均线 + 布林带（金叉后 2 天内 + 中轨上方 + 不近上轨） | **+36.21%** | **1.53** | 10.28% | 3 万账户主策略 |

回测区间：HS300 / 2025-05-12 ~ 2026-05-12 / 初始资金 3 万 / 默认风控 + 现实成本。
旧策略源码和配置保留在仓库中作为实验记录，但不再注册到 CLI / scan 的生产策略入口。
...
│   ├── strategy/                # 2 个生产策略 + 历史策略源码 + base + registry
...
回测条件统一：HS300 / 2023-01-01 ~ 2026-05-07 / 3 万初始资金 / 默认风控。
...
- `scripts/sensitivity_test.py` — 成本敏感性历史实验
